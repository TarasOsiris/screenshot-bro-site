...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "li", text: "The inspector's controls are grouped into collapsible sections. Click anywhere on a section's header to open or close it; hold **⌥** while clicking to open or close every section at once. How you leave them is remembered." },
      { kind: "li", text: "With **Edit selected shapes in the inspector** turned on in Settings, selecting a shape swaps these controls for the shape's own. Click the row name at the top of the inspector, or press **Esc**, to come back." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "p", text: "Prefer a sidebar? Turn on **Edit selected shapes in the inspector** in Settings ▸ General. The same controls then move into the inspector as collapsible sections (hide the inspector and the bar comes back), a multi-selection gets align and distribute buttons, and the **Insert** menu in the toolbar adds shapes." },
      ...
      { kind: "li", text: "When they are all the same kind it offers that kind's controls too, and applies each change to every shape: **Change Device** for devices, font / weight / color / alignment / Italic / Uppercase for text, corner radius, star points, SVG custom color, outline, shadow, opacity, rotation, and **Clip to Frame**." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "li", text: "Blue **alignment guides** appear while moving or resizing to show which edges are aligned." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "h", text: "Seeing what a language overrides" },
      { kind: "li", text: "In the inspector, every control this language overrides is tinted amber, so you can see at a glance what differs from the base language. The properties bar tints the position, size and image controls the same way." },
      { kind: "li", text: "A section header shows an amber count when properties inside it are overridden — so a collapsed section still tells you something is there." },
      { kind: "li", text: "The selected shape's summary sits beside its name at the top of the inspector: the language and how many properties it overrides. Its **reset** button opens a menu listing each overridden property, so you can put one back to the base language or reset them all." },
      { kind: "li", text: "Each row header shows an amber count of the shapes that language overrides; click it to select them." },
      { kind: "li", text: "Hovering a shape the current language overrides outlines it in amber instead of the usual accent color." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "li", text: "Anything that would block the upload is listed in a banner above the plan, one line per row — every language still missing an App Store locale is named on that one line, not repeated. Where a single change would settle it, the line carries a **Fix** button that makes it: ticking the App Store locales the row already matches, or switching the row to the detected display type." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "li", text: "Enter your app's **package name** (for example `com.example.myapp`). Google Play has no way to list your apps, so the name is typed rather than picked — the clock button offers the ones you used before." },
      { kind: "li", text: "**Next** checks the package before going on, so a typo or a service account without access is caught here instead of halfway through an upload." },
      { kind: "li", text: "Each row has an include switch, an **Upload as** image type (auto-detected from the row's size — the ⓘ button shows the sizes Play accepts), and a checkbox per language." },
      { kind: "li", text: "Each project language is matched to the Google Play listing language shown beside it. Play keeps one set of screenshots per language, so when two project languages match the same one — **English** and **English (US)** both reach `en-US` — only one can be checked. A language Play doesn't publish in reads **Not on Play** and is skipped." },
      ...
      { kind: "li", text: "Uploading **replaces** what is already on the listing: for every language and image type you selected, the existing screenshots are deleted and the new ones uploaded. The confirmation says so before anything is sent." },
      ...
      { kind: "li", text: "Use it to walk through the whole upload flow before you have credentials set up. While demo mode is on, the imported service account is ignored, the package field starts on a sample name, and the package check always passes." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { kind: "li", text: "**Edit selected shapes in the inspector** — shows a selected shape's properties in the sidebar instead of the bar below the canvas." },
      ...
      { kind: "li", text: "Links to rate the app, contact support, visit the website, or follow along on X, Threads, and Discord — plus **More Apps** from the same developer." },
    ],
  },
  {
    id: "pro-features",
